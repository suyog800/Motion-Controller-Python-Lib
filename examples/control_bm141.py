...
# IQ
try:
    drv.set_iq_gains(kp=0.04426, ki=20.07209846)
    print("Iq gains set")
...
# VELOCITY
try:
    drv.set_velocity_gains(kp=int(1702), ki=int(2212))
...
try: 
    ...
    drv.set_accel_decel(accel = 20*100, decel = 20*100)
except Exception as e:
...
for i in range(0,10):
    if (i%2 == 0):
        print("cycle:", i)
        drv.set_accel_decel(accel = 20*100, decel = 20*100)
        drv.set_target_position_and_velocity(position=-60.0,velocity=2000.0,force_pdo = True)
        print("going to -60")
        t = (60)*60/2000
        time.sleep(t+5)
    else:
        drv.set_accel_decel(accel = 18*100, decel = 18*100)
        drv.set_target_position_and_velocity(position=0.0,velocity=1500.0,force_pdo = True)
        print("going to 0")
        t = (60)*60/1500
        time.sleep(t+5)
