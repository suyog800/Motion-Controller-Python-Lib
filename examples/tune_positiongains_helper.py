...
pos_kp_gain = 1
try:
    while pos_kp_gain < 65535:
        drv.set_controlword(0x000F)   # Enable voltage, operation enable
        print("Controlword set to 0x000F (drive started).")

        drv.set_position_gains(kp=int(pos_kp_gain))
        print("position Kp:", int(pos_kp_gain))
        print("\n\n")
        drv.set_target_position_and_velocity(position=5.0, velocity=400)
        time.sleep(4)
        drv.set_target_position_and_velocity(position=0.0, velocity=400)
        time.sleep(8)
        
        drv.set_controlword(0x0000)

        # Increase by 30%
        pos_kp_gain = (pos_kp_gain * 1.3)
